...
export class ExpensesService {
  ...
  async getClientByUserId(userId: string) {
    ...
    if (!client.expenseModuleEnabled) {
      throw new NotFoundException('Módulo de despesas não habilitado para este cliente');
    }

    return client;
  }

  async getAccountantByUserId(userId: string) {
  ...
  async findAllForClient(queryDto: QueryExpensesDto, clientId: string) {
  ...
  async createForClient(
    ...
    // Check expense limit before creating
    ...
    if (expenseDate > today) {
      throw new BadRequestException('A data da despesa não pode ser no futuro');
    }
  ...
  async findAllForAccountant(queryDto: QueryExpensesDto, accountantId: string) {
  ...
  async findOneForAccountant(id: string, accountantId: string) {
    const expense = await this.prisma.expense.findFirst({
      where: {
        id,
        client: {
  ...
  async exportToExcel(expenses: any[], clientName?: string): Promise<Buffer> {
  ...
  async exportToPDF(expenses: any[], clientName?: string): Promise<Buffer> {
